---
description: >-
  How the app's major subsystems fit together — layers, background isolates,
  state management, and startup order.
---

# Architecture Overview

{% hint style="info" %}
This page is adapted from `docs/ARCHITECTURE.md` in the [`bluebubbles-app`](https://github.com/BlueBubblesApp/bluebubbles-app) repository. If it ever drifts from the source, the file in the repo is the source of truth.
{% endhint %}
...
See [Theming and Components](theming-and-components.md) for the concrete mechanics — services, extensions, and data model — that this skin system and the app's shared widgets are built on.
...
`CustomState` manages the paired `GetxController`'s lifecycle: it registers an `updateWidget` callback in `initState()` and deletes the controller (`Get.delete<S>(tag: _tag)`) in `dispose()` unless `forceDelete` was set to `false`.
...
{% hint style="warning" %}
Web support exists in the codebase but is deprecated — don't design new features for it.
{% endhint %}
...
`lib/services/backend/sync/` coordinates bringing the local DB up to date with the server: `SyncService` is the single entry point and picks between a `FullSyncManager` (bulk fetch on first setup or full resync) and `IncrementalSyncManager` (resumable delta sync since the last run), plus dedicated `ChatSyncManager` and `HandleSyncManager` for narrower syncs. All managers share a `SyncStatus` lifecycle (`IDLE → IN_PROGRESS → COMPLETED_SUCCESS`/`COMPLETED_ERROR`, with a `STOPPING` state for user cancellation) exposed as `Rx` fields for UI progress display. Incremental sync normally runs inside `IncrementalSyncIsolate` (see Background Isolate System above) rather than blocking the main thread.
